...
def show():
    ...
    # === Frame component ===#
    title = Label(frame1, text=("Welcome  Admin !!"), font=("times new roman", 30, "italic","bold" ), bg="pink", fg="black").place(x=50, y=30)
    ...
    frame.place(x=5,y=400,width=1120,height=50)
    ...
    def update_record():
        ...
        try:
            ...
            p7_entry.configure(state=NORMAL)
            p1_entry.configure(state=NORMAL)
            p8_entry.configure(state=NORMAL)
            p9_entry.configure(state=NORMAL)
            cur1.execute("update stud set id=%s,fname=%s,lname=%s,Mob=%s,email=%s,que=%s,ans=%s where uname=%s",(p1_entry.get(),p2_entry.get(),p3_entry.get(),p4_entry.get(),p5_entry.get(),p8_entry.get(),p9_entry.get(),x))
            con.commit()
        except:
            pass
    ...
    def delete_record():
        ...
        cur1=con.cursor()
        try:
            selected=my_game.selection()[0]
            my_game.delete(selected) 
            p6_entry.configure(state=NORMAL)
            cur1.execute("delete from stud where uname=%s",p6_entry.get())
            con.commit()
            update_record()
        except:
            pass
    ...
    #Buttons
    select = Button(frame1,text="Select Record",bg="black",fg="white",font=("Comic Sans MS",13,"italic"),cursor="hand2",command=select_record).place(x=170,y=480,width=150,height=40)
    delete=Button(frame1,text="Delete Record",bg="black",fg="white",font=("Comic Sans MS",13,"italic"),cursor="hand2",command=delete_record).place(x=330,y=480,width=150,height=40)
    Uupdate = Button(frame1,text="Update Record",bg="black",fg="white",font=("Comic Sans MS",13,"italic"),cursor="hand2",command=update_record).place(x=490,y=480,width=150,height=40)
    add_btn=Button(frame1,text="Add Record",bg="black",fg="white",font=("Comic Sans MS",13,"italic"),cursor="hand2",command=add).place(x=650,y=480,width=150,height=40)
    signout=Button(frame1,text="Sign Out",bg="black",fg="white",font=("Comic Sans MS",13,"italic"),cursor="hand2",command=login).place(x=810,y=480,width=150,height=40)
